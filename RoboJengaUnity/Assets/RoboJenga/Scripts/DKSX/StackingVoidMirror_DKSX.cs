...
public class StackingVisionVoidMirror : IStackable
{
    ...
    public ICollection<Pose> Display { get; }
    ...
    PickAndPlaceData BuildVoidMirror()
    {
        ...
        //Sorting the input list in subsets of Z
        var zSortedTower = _pickTiles.GroupBy(p => p.position.z)
            .Select(group => group.ToList()).ToList();

        //Cycle through the lists to define the occupied positions, according to layer orientation
